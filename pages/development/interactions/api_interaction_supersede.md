...
|----------------------|-------|-------|
| `Accept`      | The `Accept` header indicates the format of the response the client is able to understand, if set, this must be either <code class="highlighter-rouge">application/fhir+json</code> or <code class="highlighter-rouge">application/fhir+xml</code>. | OPTIONAL |
...
4. The NRL will transactionally:
   ...
   3. Mark the original DocumentReference as "superseded".
...
If both `target.reference` and `target.identifier` properties are populated then the NRL will use the `target.reference property` to resolve the DocumentReference. If a DocumentReference is found, then the MasterIdentifier of the returned DocumentReference must match the identifier in the `relatesTo` collection.
