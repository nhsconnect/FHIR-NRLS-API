...
All pointers which allow for retrieval in the `Vaccination List FHIR STU3 v1` record format MUST retrun information conforming to the guidance and requirements specified on this page.
...
| --- | --- | --- |
| List | 1..1 | Container for list of immunization for the patient.|
| CareConnect-Immunization-1 | 0..* | Immunizations listed will be included in the bundle as Immunization resources.<br/><br/>The cardinality allows for zero immunizations to be included to allow for where pointer maintenance may not align with data management. For example: if pointers are maintained as an overnight batch process, but a vaccination could be removed at any time in the day, this may result in a pointer pointing to an empty list.<br/><br/>Providers **MUST** remove pointers which will not return any immunizations, as soon as possible. |
| CareConnect-Patient-1 | 1..1 | The patient resource identifies the patient which the immunizations relate to. |
...
| Resource | Cardinality | Description |
|---|---|---|
| CareConnect-Encounter-1 | 0..* | Encounter resources may be included to give context to the immunization. |
...
| CareConnect-Location-1 | 0..* | Location resources may be included to add context to the location where immunizations occurred. |
| CareConnect-Practitioner-1 | 0..* | Practitioner details may be included in the bundle in relation to immunizations.<br/><br/>**Note:** it is important to consider Information Governance when including practitioner personal data within information shared with other organisations. |
...
The Immunization resources included as part of the returned information SHALL conform to the `CareConnect-Immunization-1` constrained FHIR profile and the additional population guidance as per the table below:
...
| vaccineCode | 1..1 | Immunization.vaccineCode SHALL use a value from the CareConnect-VaccineCode-1 value set. Where the vaccineCode is not known, such as when a vaccination is reported a unknown value should be used. |
...
| reportOrigin | 0..1 | If the vaccination was reported then the original source should be included |
