...
This document is intended for use by software developers looking to build a conformant NRLS FoT API interfaces or consumer application utilising the Spine Security Proxy (SSP).
...
The Spine Security Proxy (SSP) is a forward proxy which will be used as a front-end to control and protect access to NRLS systems that will be exposing records as defined by the NRLS programme.  It will provide a single security point for both authentication and authorisation for consumer and provider systems. Additional responsibilities include auditing of all requests and transaction logging for performance and commercial remuneration purposes. In the first instance the proxy will be made available on the N3 network but it is envisaged that in a later phase of the NRLS programme access via the public internet will also be supported.
...
| 2.   | Locate the organisation that holds the record. | PDS or FHIR RLS | The consumer system SHALL use organisation details previously retrieved from PDS. <br/><br/>OR (when available^) The consumer system SHALL use organisation details retrieved from the FHIR RLS. |
...
| 5.   | Lookup the patient’s sharing preferences. | PPR | Out of scope for NRLS FoT. |
| 6.   | Lookup organisational data sharing. | DSAR | Interim solution as part of SSP.  Strategic solution out of scope for NRLS FoT. |
...
<!--
<sup>^</sup> will be made available after the initial NRLS FoT go-live.
...
-->
> Note: whilst composite routing services inside the proxy (i.e. Step 3b) is under consideration this is not in-scope for NRLS FoT.
