...
The NRL is based on the [Registry-Repository:](https://developer.nhs.uk/library/architecture/integration-patterns/registry-repository/) pattern which separates the storage and retrieval of a record from data that describes its location. 
...
The <b>NRL</b> is acting as a registry with the repository function carried out by so-called <b>Providers</b>. Providers are systems external to the NRL that expose records for retrieval. Pointers are created by Providers to signpost a record that is intended to be exposed for retrieval. 
